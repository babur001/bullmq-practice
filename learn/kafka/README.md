...
**transactional outbox**, and **sagas**. This course builds _directly_ on that — we won't
...
> A BullMQ queue is a **to-do list** — a job is consumed and _removed_. A Kafka topic is a
> **durable, replayable log** — an event is _retained_ and any number of readers replay it
...
BullMQ vocabulary you already own onto Kafka's, and flags the _false friends_ (words that
...
| 02  | Producing: keys, partitioning & ordering                 | How records route to partitions, `acks` & durability, the _per-partition_ ordering guarantee                                           |
...
| 08  | Event Sourcing & CQRS                                    | State _is_ the log; projections, read models, rebuilding state by replay                                                               |
